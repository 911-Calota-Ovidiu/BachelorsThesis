...
export abstract class ConnectionObject{
    ...
    constructor(nodeId: number, startX : number, startY : number, endX : number, endY : number, color: string, type: string, size: number,){
        this.startX = startX;
        this.startY = startY;
        this.endX = endX;
        this.endY = endY;
...
export abstract class NodeObject {
    ...
    constructor(nodeId : number, x : number, y : number, type : string, color : string, size: number, activationMode: number) {
        this.nodeId = nodeId;
        this.editorShowing = false;
        this.x = x;
        this.y = y;
...
export function getNodeFromName(name: string, x: number, y: number, nodeId : number){
    switch (name){
        case "Source" : return new Source(nodeId, x, y, name, "black", 2, 1);
        case "Drain" : return new Drain(nodeId, x, y, name, "black", 2, 1);
        case "Pool" : return new Pool(nodeId, x, y, name, "black", 2, 1);
        case "Gate" : return new Gate(nodeId, x, y, name, "black", 2, 1);
        case "Trader" : return new Trader(nodeId, x, y, name, "black", 2, 1);
        case "Converter" : return new Converter(nodeId, x, y, name, "black", 2, 1);
        case "EndCondition" : return new EndCondition(nodeId, x, y, name, "black", 2, 1);
        case "Text" : return new TextComponent(nodeId, x, y, name, "black", 2, 1);
        case "Register" : return new Register(nodeId, x, y, name, "black", 2, 1);
        default: throw new Error("No selection");
    }
}

export function getConnectionFromName(nodeId: number, name: string, startX: number, startY: number, endX: number, endY: number) {
    switch (name) {
        case "ResourceNode" : return new  ResourceConnection(nodeId, startX,startY,endX,endY,"black", "ResourceConnection",2);
        case "StateNode" : return new  StateConnection(nodeId, startX,startY,endX,endY,"black","StateConnection",2);
        default: throw new Error("No selection");
    }
}